    ...
    public class GOAPAgentVisualEditor : Editor
    {
        public override void OnInspectorGUI()
        {
            ...
            foreach (GOAPAction a in agent.gameObject.GetComponent<GOAPAgent>().baseSettings.actions)
            {
                ...
                foreach (KeyValuePair<AString_SO, int> p in a.baseSettings.preConditions)
                {
                    pre += p.Key + ", ";
                }

                foreach (KeyValuePair<AString_SO, int> e in a.baseSettings.afterEffects)
                {
                    eff += e.Key + ", ";
                }
            ...
            {
                foreach (KeyValuePair<AString_SO, int> sg in g.Key.subGoals)
                    GUILayout.Label("=====  " + sg.Key);
            }

            GUILayout.Label("Beliefs: ");

            foreach (KeyValuePair<AString_SO, int> sg in agent.gameObject.GetComponent<GOAPAgent>().baseSettings.localStates.GetStates())
            {
                GUILayout.Label("=====  " + sg.Key);
            }
